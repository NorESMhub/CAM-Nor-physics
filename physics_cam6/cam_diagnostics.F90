...
!AL
    if ( dycore_is('LR') )then
      call addfld ('DME'//cnst_name(       1),(/ 'lev' /), 'A','kg/kg/s', &
           trim(cnst_name(       1))//' dme adjustment tendency (FV) ')
      if (ixcldliq > 0) then
         call addfld ('DME'//cnst_name(ixcldliq),(/ 'lev' /), 'A','kg/kg/s', &
            trim(cnst_name(ixcldliq))//' dme adjustment tendency (FV) ')
      end if
      if (ixcldice > 0) then
        call addfld ('DME'//cnst_name(ixcldice),(/ 'lev' /), 'A','kg/kg/s', &
             trim(cnst_name(ixcldice))//' dme adjustment tendency (FV) ')
      end if
!AL
      call addfld ('DME'//cnst_name(ixcldnc),(/ 'lev' /), 'A','#/kg/s ',   &
           trim(cnst_name(ixcldnc))//' dme adjustment tendency (FV) ')
      call addfld ('DME'//cnst_name(ixcldni),(/ 'lev' /), 'A','#/kg/s ',   & 
...
!AL
    ...
    if ( history_budget ) then
      ...
      if ( dycore_is('LR') )then
        call add_default('DME'//cnst_name(1)       , history_budget_histfile_num, ' ')
        if (ixcldliq > 0) then
           call add_default('DME'//cnst_name(ixcldliq), history_budget_histfile_num, ' ')
        end if
        if (ixcldice > 0) then
          call add_default('DME'//cnst_name(ixcldice), history_budget_histfile_num, ' ')
        end if
      end if
...
!===============================================================================
  ...
  subroutine diag_surf (cam_in, cam_out, state, pbuf)
    ...
    if (moist_physics) then
      ...
      call qsat(cam_in%tref(:ncol), state%ps(:ncol), tem2(:ncol), ftem(:ncol), ncol)
...
!AL
    ...
    if (dycore_is('LR')) then
      ...
      if ( cnst_cam_outfld(       1) ) then
        call outfld ('DME'//cnst_name(       1), tmp_q     , pcols, lchnk)
      end if
      if (ixcldliq > 0) then
        if ( cnst_cam_outfld(ixcldliq) ) then
          call outfld ('DME'//cnst_name(ixcldliq), tmp_cldliq, pcols, lchnk)
        end if
      end if
      if (ixcldice > 0) then
        if ( cnst_cam_outfld(ixcldice) ) then
          call outfld ('DME'//cnst_name(ixcldice), tmp_cldice, pcols, lchnk)
        end if
      end if
!AL
      ...
      if ( cnst_cam_outfld(ixnumliq) ) call outfld ('DME'//cnst_name(ixnumliq), tmp_cldnc, pcols, lchnk)
      if ( cnst_cam_outfld(ixnumice) ) call outfld ('DME'//cnst_name(ixnumice), tmp_cldni, pcols, lchnk)
!AL
   end if

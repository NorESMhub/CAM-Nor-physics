...
!+tht update cam6_3_100 -> cam6_3_100_ebudget_dev_update
...
!-tht update cam6_3_100 -> cam6_3_100_ebudget_dev_update
 ...
 !  call shr_sys_flush(iulog) ! bad idea
...
!+tht update cam6_3_100 -> cam6_3_100_ebudget_dev_update
     ...
     !     call physics_dme_adjust_BAB(state, tend, qini, dt)
           call physics_dme_adjust_BAB(state, tend, qini, liqini, iceini, dt)
!-tht update cam6_3_100 -> cam6_3_100_ebudget_dev_update
      end if
...
!+tht update cam6_3_100 -> cam6_3_100_ebudget_dev_update
 !subroutine physics_dme_adjust_BAB(state, tend, qini, dt)
  subroutine physics_dme_adjust_BAB(state, tend, qini, liqini, iceini, dt)
...
!+tht ebudget_dev update
    ...
    !(tht:N.B. if all was programmed correctly, this should not be necessary?)
...
!+tht ebudget_dev update (John/Peter's code, plus tht comments)
    else
     do k=1,pver
     !+tht this is coded like shit and obviously causes errors unless m_const=(1,2,3) 
     ...
     !+tht the line following below, after this comment, is incorrect:
     !+tht  case a): state%q is a mixing ratio r
     !+tht           then
     !                fdq =  (1+tot_water(2))/(1+tot_water(1))
     !               is the correct factor, which must be applied to dp only, and NOT to state%q
     !+tht  case b): state%q is a specific humidity q=r/(1+r)
     !+tht           then
     !+tht            fdq = 1+(tot_water(2)-tot_water(1))/(1.-tot_water(2))
     !+tht           is the correct factor for dp, and its inverse for q 
     !+tht  I assume case b) is correct (depends on set_dry_to_wet call in physpkg)
       !fdq(:ncol) = 1._r8 + tot_water(:ncol,2) - tot_water(:ncol,1)
        fdq(:ncol) = 1._r8 +(tot_water(:ncol,2) - tot_water(:ncol,1))/(1._r8-tot_water(:ncol,2))
     !-tht
        ! adjust constituents to conserve mass in each layer
        do m = 1, pcnst
          state%q(:ncol,k,m) = state%q(:ncol,k,m) / fdq(:ncol)
     ...
     enddo
    !+tht N.B. now, to maintain TD consistency, the hydrostatic pressure work
    !+tht      heating term -pd(alpha) needs to be added. 
    !+tht      Regardless of hydrostatic pressure (coordinate) and TD assumptions,
    !+tht      alpha always involves dry air and water vapour ONLY (with corrections
    !+tht      if condensates occupy a finite volume).
    !+tht See physics_dme_adjust_THT below for how to do that.
    endif
!-tht ebudget_dev update
    if ( waccmx_is('ionosphere') .or. waccmx_is('neutral') ) then
...
!+tht ebudget_dev update
 !subroutine physics_dme_adjust_THT(state, tend, qini, dt, eflx, ent_tnd, ohf_adjust, ocnfrac, sst, ts)
  subroutine physics_dme_adjust_THT(state, tend, qini, liqini, iceini, dt, eflx, ent_tnd, ohf_adjust, ocnfrac, sst, ts)
    use air_composition, only: dry_air_species_num,thermodynamic_active_species_num
    use air_composition, only: thermodynamic_active_species_idx
...
!+tht 17.11.2015 option to use virtual temperature for T update
   ...
   !real(r8) :: xr(pcols)                        ! recipr. density (ebudget update)
...
!+tht ebudget_dev update
     ! mdq    (:ncol,k)= state%q(:ncol,k,1) - qini(:ncol,k) ! only water-vapor mass change considered 
       tot_water(:ncol,2) = 0.0_r8
       tot_water(:ncol,1) = qini(:ncol,k) !initial total H2O
         do m_cnst=dry_air_species_num+1,thermodynamic_active_species_num
          m = thermodynamic_active_species_idx(m_cnst)
          if (m.eq.2) tot_water(:ncol,1)=tot_water(:ncol,1)+liqini(:ncol,k)
          if (m.eq.3) tot_water(:ncol,1)=tot_water(:ncol,1)+iceini(:ncol,k)
          tot_water(:ncol,2) = tot_water(:ncol,2)+state%q(:ncol,k,m)
        end do
...
!+tht this was set here to ensure rr*rairv is the physically correct rairv
        rr(:ncol) = (1._r8+(zvir+1._r8)*.5_r8*(qini(:ncol,k)+state%q(:ncol,k,1))) &
                     /(1._r8+.5_r8*(qini(:ncol,k)+state%q(:ncol,k,1)))
       else
        rr(:ncol) = 1._r8 ! same actually but /(1+total water) instead of /(1+wv)
       endif

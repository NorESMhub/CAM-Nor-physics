...
!moved from moistconvection.F90
  ...
  !real(r8),parameter :: & !<--updt to 6.3.41
   real(r8)           :: & !<--updt to 6.3.41
! standard parameters:
...
! updt to 6.3.100-->
   logical :: lparcel_pbl 
   real(r8):: dmpdz_param
! new parameter (hurray!): scaling of PBL height as parcel mixing length when lparcel_pbl=True 
...
!+tht give a honest and transparent list of ALL parameters
    ...
    call endrun('**** ZM_CONVI : tht_tweaks must be T in order to use second_call or retrigger ****')
     ...
     write(iulog,*) '       Entrainment rate in initial test plume for CAPE: dmpdz_param',dmpdz_param!<--updt to 6.3.41
  tentrm=-dmpdz_param !<--updt to 6.3.41
...
! updt to 6.3.100-->
                 !tpert   , org    , landfrac , dmpdz) !+tht DMPDZ 3D
                  zi      ,zs      ,tpert   ,org     ,landfrac  ,dmpdz)
!<-- updt 6.3.100
   end if
...
! updt to 6.3.100-->
! Use z interface/surface relative values for PBL for parcel calculations.
   real(r8), intent(in) :: zi(pcols,pver+1)
...
!+tht: use total mse -- moist thermo
  !hmn_lev(:ncol,1:pver) = cp*t(:ncol,1:pver) + grav*z(:ncol,1:pver) + rl*q(:ncol,1:pver)
   hmn_lev(:ncol,1:pver) =(cp+q(:ncol,1:pver)*cpliq)*t(:ncol,1:pver)/(1._r8+q(:ncol,1:pver)) &
                + (1._r8+q(:ncol,1:pver)/eps1)/(1._r8+q(:ncol,1:pver))*grav*z(:ncol,1:pver) &
...
! Mix profile over vertical length scale of 0.5*PBLH.
   ...
   do i = 1,ncol ! Loop columns
      do k = pver,msg + 1,-1
         if (zi(i,k+1)<= parcel_dz(i)) then ! Has to be relative to to near-surface layer center elevation
            ipar = k
...
!<-- updt to 6.3.41
   do i = 1,ncol ! Initialise LCL variables.
      lcl(i) = mx(i)
      tl(i) = t(i,mx(i))
      ql(i) = q(i,mx(i)) !<--updt to 6.3.41
      pl(i) = p(i,mx(i))
   end do
endif !<--updt tp 6.3.41
!
! main buoyancy calculation.
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
real(r8), intent(inout), dimension(pcols) :: ql ! Actual humidity of LCL !<--updt to 6.3.41
...
!+tht: formulate dilution on enthalpy not on entropy
         if (tht_tweaks) then
          sp0(i)  = enthalpy(tl(i),pl(i),qtp0(i),zl(i))  ! Parcel launch entropy could be a mixed parcel.
         else

...
subroutine dyn_readnl(nlfilename)
   ...
   logical :: fv_am_fix_taper =.false. ! apply global AM fixer level by level
   real(r8):: fv_am_fix_tpr_h =95e2_r8 ! apply AM fixer tapering above this pressure
   real(r8):: fv_am_fix_tpr_w =10e2_r8 ! taper AM fixer to zero within this pressure interval
   ...
   namelist /dyn_fv_inparm/ fv_nsplit, fv_nspltrac, fv_nspltvrm, fv_iord, fv_jord, &
                            ...
                            fv_am_fixer, fv_am_fix_lbl, & 
                            fv_am_fix_taper, fv_am_fix_tpr_h, fv_am_fix_tpr_w, &
                            fv_am_diag, fv_high_altitude, &
   ...
   call mpi_bcast(fv_am_fix_taper, 1, mpi_logical, mstrid, mpicom, ierr)
   if (ierr /= 0) call endrun(sub//": FATAL: mpi_bcast: fv_am_fix_taper")

   call mpi_bcast(fv_am_fix_tpr_h, 1, mpi_logical, mstrid, mpicom, ierr)
   if (ierr /= 0) call endrun(sub//": FATAL: mpi_bcast: fv_am_fix_tpr_h")

   call mpi_bcast(fv_am_fix_tpr_w, 1, mpi_logical, mstrid, mpicom, ierr)
   if (ierr /= 0) call endrun(sub//": FATAL: mpi_bcast: fv_am_fix_tpr_w")
   ...
   dyn_state%am_fix_taper  = fv_am_fix_taper
   dyn_state%am_fix_tpr_h  = fv_am_fix_tpr_h
   dyn_state%am_fix_tpr_w  = fv_am_fix_tpr_w
   ...
   if (masterproc) then
      ...
      write(iulog,*)'  High-altitude taper for AM fixer (fv_am_fix_taper)  = ', fv_am_fix_taper
      if (fv_am_fix_taper.eq..true.) then
      write(iulog,*)'          Tapering starts at   (fv_am_fix_tpr_h) [Pa] = ', fv_am_fix_tpr_h
      write(iulog,*)'          Taper to zero within (fv_am_fix_tpr_w) [Pa] = ', fv_am_fix_tpr_w
      endif
...
#include "mpif.h"
   ...
   real(r8) :: am_fix_tpr_h, am_fix_tpr_w ! height and width of taper
   real(r8) :: tmpsum(1,2)
   real(r8) :: tmpresult(2)
   real(r8) :: am0, am1, me0
   ...
   real(r8) :: ptapk, xdlt2
   real(r8)            :: ptap =9000._r8
   real(r8)            :: dptap=1000._r8
   ...
   am_fix_taper=dyn_state%am_fix_taper
   am_fix_tpr_h=dyn_state%am_fix_tpr_h
   am_fix_tpr_w=dyn_state%am_fix_tpr_w
   am_diag    = dyn_state%am_diag
!+tht taper
   if(am_fix_taper)then !use namelist values
    ptap = am_fix_tpr_h - am_fix_tpr_w/2._r8
    dptap= am_fix_tpr_w
   endif                !otherwise defaults
!-tht
   pi   =  constants%pi
   om   =  constants%omega
   ae   =  constants%ae

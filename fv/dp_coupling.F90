...
!
   ...
   use constituents,  only: cnst_get_ind !+tht Needed to access constituent indices
...
! !USES:
   ...
   !use cam_thermo,     only: cam_thermo_update
    use cam_thermo,     only: cam_thermo_dry_air_update !ebudget update
    ...
    use air_composition,only: dry_air_species_num !ebudget update
!-----------------------------------------------------------------------
    implicit none
!-----------------------------------------------------------------------
! !INPUT PARAMETERS:
...
! required q change in bottom level rescaled to mass fraction in 2nd level
      ...
      !if ( waccmx_is('ionosphere') .or. waccmx_is('neutral') ) then
       if ( dry_air_species_num>0 ) then !ebudget update
!------------------------------------------------------------
...
!-----------------------------------------------------------------------------
        !call cam_thermo_update(phys_state(lchnk)%q, phys_state(lchnk)%t, lchnk, ncol)
         call cam_thermo_dry_air_update(phys_state(lchnk)%q, phys_state(lchnk)%t, lchnk, ncol) !ebudget update
       endif
...
! Compute initial geopotential heights
       call geopotential_t (phys_state(lchnk)%lnpint, phys_state(lchnk)%lnpmid  , phys_state(lchnk)%pint  , &
                           ...
                           !phys_state(lchnk)%t     , phys_state(lchnk)%q(:,:,1), rairv(:,:,lchnk), gravit, zvirv, &
                            phys_state(lchnk)%t     , phys_state(lchnk)%q(:,:,:), rairv(:,:,lchnk), gravit, zvirv, & !ebudget update
